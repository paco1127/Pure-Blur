...
def process_tracking(frame, detector, tracker, args, frame_id, fps):
    ...
    if outputs is not None:
        ...
        # if face is unknown, blur the face with a Gaussian filter
        for i in range(len(tracking_bboxes)):
            print(tracking_ids[i], id_face_mapping)
            if id_face_mapping != {} and tracking_ids[i] in id_face_mapping and id_face_mapping[tracking_ids[i]] == "UN_KNOWN":
                x1, y1, x2, y2 = tracking_bboxes[i]
                # to int
                x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
                face = frame[y1:y2, x1:x2]

                # Check if the face region is not empty or None
                if face is not None and len(face) > 0 and face.size != 0:
                    face = cv2.GaussianBlur(face, (99, 99), 30)
                    frame[y1:y2, x1:x2] = face
        
        tracking_image = frame
